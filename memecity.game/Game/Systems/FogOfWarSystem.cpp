...
void FogOfWarSystem::run(memecity::engine::ecs::EntityManager& em) const
{
	auto components = em.query<DrawableComponent>()
		...
		return c.get_texture().get_render_rect().x > -100
			&& c.get_texture().get_render_rect().x < multimedia_manager.get_screen_width() + 100
			&& c.get_texture().get_render_rect().y > -100
			&& c.get_texture().get_render_rect().y < multimedia_manager.get_screen_height() + 100;
	}).to_vector();
