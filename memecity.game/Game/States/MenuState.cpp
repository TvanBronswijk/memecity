	...
	: State(sm), _context(&gc)
{
	...
	menu = memecity::engine::ui::menu::MenuBuilder(gc.get_multimedia_manager())
		...
		.with_menu_item("Start Game", nullptr, [&](auto& menu_item) { next<LoadingState>(gc, [&](auto& ctx, auto& listener) { replace<GameState>(ctx, GameLoader(ctx, 128, 128).build(listener)); }, gc.get_multimedia_manager()); })
