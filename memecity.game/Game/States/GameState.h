...
public:
	GameState(memecity::engine::state::StateManager& sm, GameManager::GameContext& gc, bool load_from_file, std::string save_location)
		: State(sm), _context(&gc), _hud(_context->get_multimedia_manager(), _context->get_multimedia_manager().get_texture(assets::sprites::DARK_BACKGROUND, 0, 0, _context->get_multimedia_manager().get_screen_width(), 100), 0, 0), _load_from_file(load_from_file),_save_location(save_location),_map_number(0)
	{
