	...
	public:
		...
		void write(models::City& c, const Node& n) const override {
			iterate(n, [&](int x, int y) {
				int xoff = x - n.x;
				int yoff = y - n.y;
				c.tiles(x, y) = xoff < _prefab.width && yoff < _prefab.height ? _prefab.tiles(xoff, yoff) : '-';
				c.objects(x, y) = xoff < _prefab.width && yoff < _prefab.height ? _prefab.objects(xoff, yoff) : '\0';
			});
		}
	};
}
