...
namespace memecity::engine::ui::menu {
	...
	public:
		...
		: selected_menu_items_index(0), multimedia_manager(multimedia_manager), input_manager(input_manager), is_locked(false), parent(parent), title(title)
		{
		...
		MenuItem& create_menu_item(Args&&... args)
		{
			...
			return *menu_items.emplace_back(std::move(std::make_unique<MenuItem>(multimedia_manager, input_manager, *this, std::forward<Args>(args)...)));
		}
