...
#include <Engine.h>
#include "MenuItem.h"
#include "../../../memecity.game/Assets.h"
...
namespace memecity::engine::ui::menu {
	...
	private:
		...
		std::vector<std::unique_ptr<MenuItem>> read_only_menu_items;
	...
	public:
		...
		{
			title_texture = multimedia_manager.get_text_texture(title, assets::fonts::DEFAULT_FONT, 36, { 255,255,255 });
		}
		...
		{
			static_assert(std::is_constructible<MenuItem, MultimediaManager&, Menu&, Args...>::value, "The requested type cannot be constructed from the arguments provided.");
			return *read_only_menu_items.emplace_back(std::make_unique<MenuItem>(multimedia_manager, *this, std::forward<Args>(args)...));
		}
